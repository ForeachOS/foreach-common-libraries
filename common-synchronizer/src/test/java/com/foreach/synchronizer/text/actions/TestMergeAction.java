...
public class TestMergeAction {
    ...
    public void testWriteToFiles() throws Exception {
        ...
        when( localizedTextFileHandler.getOutputStream( outputDir, "application1", "group1", format ) ).thenReturn( outputStream1 );
        when( localizedTextFileHandler.getOutputStream( outputDir, "application2", "group2", format ) ).thenReturn( outputStream2 );
        when( localizedTextFileHandler.getOutputStream( outputDir, "application3", "group3", format ) ).thenReturn( outputStream3 );

        when(localizedTextWriterFactory.createLocalizedTextWriter(format, outputStream1)).thenReturn( localizedTextWriter1 );
        when(localizedTextWriterFactory.createLocalizedTextWriter(format, outputStream2)).thenReturn( localizedTextWriter2 );
        when(localizedTextWriterFactory.createLocalizedTextWriter(format, outputStream3)).thenReturn( localizedTextWriter3 );
    ...
    public void testExcelFormat() throws ParseException {
        verifyExpectedFormatUsed( new String[]{"-i", "/inputDir", "-i", "/another/inputDir", "-o", "/outputDir", "-f", "EXCEL"}, LocalizedTextFormat.EXCEL );
    }
    ...
    public void testXMLFormat() throws ParseException {
        verifyExpectedFormatUsed( new String[]{"-i", "/inputDir", "-i", "/another/inputDir", "-o", "/outputDir", "-f", "XML"}, LocalizedTextFormat.XML );
    }

    @Test
    public void testDefaultFormat() throws ParseException {
        verifyExpectedFormatUsed( new String[]{"-i", "/inputDir", "-i", "/another/inputDir", "-o", "/outputDir"}, LocalizedTextFormat.XML );
    }
    ...
    private void verifyExpectedFormatUsed( String[] args, LocalizedTextFormat expectedOutputFormat ) throws ParseException {
        ...
        verify( localizedTextFileHandler ).getOutputStream( "/outputDir", "application1", "group1", expectedOutputFormat );
        verify( localizedTextWriterFactory, times(2) ).createLocalizedTextWriter( eq( expectedOutputFormat ), any( OutputStream.class ) );
    }
